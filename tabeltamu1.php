...
$tahunskr=date('Y');
    ...
    if($_POST['bulan']==''){
        $select1 = "SELECT *, DATE_FORMAT(timestamp, '%d %b %Y %H:%i:%s') as tgl FROM tamu WHERE DATE_FORMAT(timestamp, '%Y')='".$_POST['tahun']."' ORDER BY timestamp DESC";
    }
    else{
        $select1 = "SELECT *, DATE_FORMAT(timestamp, '%d %b %Y %H:%i:%s') as tgl FROM tamu WHERE DATE_FORMAT(timestamp, '%m')='".$_POST['bulan']."' AND DATE_FORMAT(timestamp, '%Y')='".$_POST['tahun']."' ORDER BY timestamp DESC";
    }
